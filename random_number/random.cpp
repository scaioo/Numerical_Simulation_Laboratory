...
void Random :: start(){
   int seed[4];
   int p1, p2;
   ifstream Primes("../random_number/Primes");
   if (Primes.is_open()){
      Primes >> p1 >> p2 ;
   } else cerr << "PROBLEM: Unable to open Primes" << endl;
   Primes.close();

   ifstream input("../random_number/seed.in");
   string property;
   if (input.is_open()){
      while ( !input.eof() ){
         input >> property;
...
double Random::Exponential(double lambda){
    // This function generates a random number with an exponential distribution
    return -log(1-Rannyu())/lambda;
}

double Random::CauchyLorentz(double mu, double Gamma){
    // This function generates a random number with a Cauchy-Lorentz distribution
