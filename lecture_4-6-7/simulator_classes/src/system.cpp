...
void System :: measure(){ // Measure properties
  ...
  vec distance;
  distance.resize(_ndim);
  double r, dr;                // position and distance between particles
  ...
  double tenergy_square=0.0;   // temporary accumulator for energy square
  ...
  if (_measure_penergy or _measure_pressure or _measure_gofr) {
    for (int i=0; i<_npart-1; i++){
      for (int j=i+1; j<_npart; j++){
        ...
        distance(1) = this->pbc( _particle(i).getposition(1,true) - _particle(j).getposition(1,true), 1);
        distance(2) = this->pbc( _particle(i).getposition(2,true) - _particle(j).getposition(2,true), 2);
        r=sqrt(pow(_particle(i).getposition(0,true),2)+pow(_particle(i).getposition(1,true),2)+pow(_particle(i).getposition(2,true),2));
        dr = sqrt( arma::dot(distance,distance) );
...
void System :: averages(int blk){
  ...
  if(_measure_cv){_average(_index_cv) = _npart*pow(_beta,2)*(_average(_index_cv)-pow(_average(_index_tenergy),2));}
  if(_measure_gofr){
  for(int i=0; i<_n_bins; i++){
    double DeltaV = 4./3.*M_PI*(pow((i+1)*_bin_size,3)-pow(i*_bin_size,3));
    _average(_index_gofr+i) /= (double(_npart)*_rho*DeltaV);
   }
  }
  _global_av  += _average;
...
void System :: set_rho(double rho){
  _rho = rho;
  return;
}
...
void System :: set_nsteps(int nsteps){
  _nsteps = nsteps;
  return;
}

void System :: set_nbl(int nbl){
  _nblocks = nbl;
  return;
}

//-----------------------------------------------------------------------//
// adding oveloaded function to specify the input and output of results  //
//-----------------------------------------------------------------------//

void System :: initialize(string input_path,string output_path){ // Initialize the System object according to the content of the input files in the ../INPUT/ directory

  int p1, p2; // Read from ../INPUT/Primes a pair of numbers to be used to initialize the RNG
  ifstream Primes(input_path +"INPUT/Primes");
  Primes >> p1 >> p2 ;
  Primes.close();
  int seed[4]; // Read the seed of the RNG
  ifstream Seed(input_path+"INPUT/seed.in");
  Seed >> seed[0] >> seed[1] >> seed[2] >> seed[3];
  _rnd.SetRandom(seed,p1,p2);

  ofstream couta(output_path+"OUTPUT/acceptance.dat"); // Set the heading line in file ../OUTPUT/acceptance.dat
  couta << "#   N_BLOCK:  ACCEPTANCE:" << endl;
  couta.close();

  ifstream input(input_path+"INPUT/input.dat"); // Start reading ../INPUT/input.dat
  ofstream coutf;
  coutf.open(output_path+"OUTPUT/output.dat");
  string property;
  double delta;
  while ( !input.eof() ){
    input >> property;
  ...
  input.close();
  this->read_configuration(input_path);
  this->initialize_velocities(input_path);
  coutf << "System initialized!" << endl;
  coutf.close();
  return;
}

void System :: initialize(string input_path,string output_path,string config_path){ // Initialize the System object according to the content of the input files in the ../INPUT/ directory
  ...
  input.close();
  this->read_configuration(config_path);
  this->initialize_velocities(config_path);
  coutf << "System initialized!" << endl;
  coutf.close();
  return;
}

void System :: initialize_velocities(string input_path){
  if(_restart and _sim_type==0){
    ifstream cinf;
    cinf.open(input_path+"INPUT/CONFIG/velocities.in");
    if(cinf.is_open()){
      double vx, vy, vz;
      for(int i=0; i<_npart; i++){
        cinf >> vx >> vy >> vz;
...
void System :: initialize_properties(string input_path,string output_path){ // Initialize data members used for measurement of properties
  ...
  ifstream input(input_path+"INPUT/properties.dat");
  if (input.is_open()){
    while ( !input.eof() ){
      input >> property;
      if( property == "POTENTIAL_ENERGY" ){
        ofstream coutp(output_path+"OUTPUT/potential_energy.dat");
        coutp << "#     BLOCK:  ACTUAL_PE:     PE_AVE:      ERROR:" << endl;
        coutp.close();
        _nprop++;
        _index_penergy = index_property;
        ...
        ofstream coutk(output_path+"OUTPUT/kinetic_energy.dat");
        coutk << "#     BLOCK:   ACTUAL_KE:    KE_AVE:      ERROR:" << endl;
        coutk.close();
        _nprop++;
        _measure_kenergy = true;
        ...
        ofstream coutt(output_path+"OUTPUT/total_energy.dat");
        coutt << "#     BLOCK:   ACTUAL_TE:    TE_AVE:      ERROR:" << endl;
        coutt.close();
        _nprop++;
        _measure_tenergy = true;
        ...
        ofstream coutte(output_path+"OUTPUT/temperature.dat");
        coutte << "#     BLOCK:   ACTUAL_T:     T_AVE:       ERROR:" << endl;
        coutte.close();
        _nprop++;
        _measure_temp = true;
        ...
        ofstream coutpr(output_path+"OUTPUT/pressure.dat");
        coutpr << "#     BLOCK:   ACTUAL_P:     P_AVE:       ERROR:" << endl;
        coutpr.close();
        _nprop++;
        _measure_pressure = true;
        ...
        ofstream coutgr(output_path+"OUTPUT/gofr.dat");
        coutgr << "# DISTANCE:     AVE_GOFR:        ERROR:" << endl;
        coutgr.close();
        ofstream coutgrf(output_path+"OUTPUT/gofr_final.dat");
        coutgrf << "# DISTANCE:     AVE_GOFR:        ERROR:" << endl; 
        coutgrf.close();
        input>>_n_bins;
        _nprop+=_n_bins;
        ...
        ofstream coutpr(output_path+"OUTPUT/magnetization.dat");
        coutpr << "#     BLOCK:   ACTUAL_M:     M_AVE:       ERROR:" << endl;
        coutpr.close();
        _nprop++;
        _measure_magnet = true;
        ...
        ofstream coutpr(output_path+"OUTPUT/specific_heat.dat");
        coutpr << "#     BLOCK:   ACTUAL_CV:    CV_AVE:      ERROR:" << endl;
        coutpr.close();
        _nprop++;
        _measure_cv = true;
        ...
        ofstream coutpr(output_path+"OUTPUT/susceptibility.dat");
        coutpr << "#     BLOCK:   ACTUAL_X:     X_AVE:       ERROR:" << endl;
        coutpr.close();
        _nprop++;
        _measure_chi = true;
        ...
        ofstream coutf;
        coutf.open(output_path+"OUTPUT/output.dat",ios::app);
        coutf << "Reading properties completed!" << endl;
        coutf.close();
        break;
      } else cerr << "PROBLEM: unknown property" << endl;
...
void System :: finalize(string output_path){
  this->write_configuration(output_path);
  _rnd.SaveSeed(output_path);
  ofstream coutf;
  coutf.open(output_path+"OUTPUT/output.dat",ios::app);
  coutf << "Simulation completed!" << endl;
  coutf.close();
...
void System :: write_configuration(string output_path){
  ofstream coutf;
  if(_sim_type < 2){
    coutf.open(output_path+"OUTPUT/CONFIG/config.xyz");
    if(coutf.is_open()){
      coutf << _npart << endl;
      coutf << "#Comment!" << endl;
      for(int i=0; i<_npart; i++){
    ...
    coutf.close();
    this->write_velocities(output_path);
  } else {
    coutf.open(output_path+"OUTPUT/CONFIG/config.spin");
    for(int i=0; i<_npart; i++) coutf << _particle(i).getspin() << " ";
    coutf.close();
  }
  return;
...
void System :: write_XYZ(string output_path,int nconf){
  ofstream coutf;
  coutf.open(output_path+"OUTPUT/CONFIG/config_" + to_string(nconf) + ".xyz");
  if(coutf.is_open()){
    coutf << _npart << endl;
    coutf << "#Comment!" << endl;
    for(int i=0; i<_npart; i++){
...
void System :: write_velocities(string output_path){
  ofstream coutf;
  coutf.open(output_path+"OUTPUT/CONFIG/velocities.out");
  if(coutf.is_open()){
    for(int i=0; i<_npart; i++){
      coutf << setw(16) << _particle(i).getvelocity(0)          // vx
            << setw(16) << _particle(i).getvelocity(1)          // vy
...
void System :: read_configuration(string input_path){
  ifstream cinf;
  cinf.open(input_path+"INPUT/CONFIG/config.xyz");
  if(cinf.is_open()){
    string comment;
    string particle;
    double x, y, z;
  ...
  if(_restart and _sim_type > 1){
    int spin;
    cinf.open(input_path+"OUTPUT/CONFIG/config.spin");
    for(int i=0; i<_npart; i++){
      cinf >> spin;
      _particle(i).setspin(spin);
    }
...
void System :: block_reset(string output_path,int blk){ // Reset block accumulators to zero
  ofstream coutf;
  if(blk>0){
    coutf.open(output_path+"OUTPUT/output.dat",ios::app);
    coutf << "Block completed: " << blk << endl;
...
void System :: averages(string output_path,int blk){
  ofstream coutf;
  double average, sum_average, sum_ave2;
  _average     = _block_av / double(_nsteps);
  if(_measure_cv){_average(_index_cv) = _npart*pow(_beta,2)*(_average(_index_cv)-pow(_average(_index_tenergy),2));}
  ...
  // POTENTIAL ENERGY //////////////////////////////////////////////////////////
  if (_measure_penergy){
    coutf.open(output_path+"OUTPUT/potential_energy.dat",ios::app);
    average  = _average(_index_penergy);
    sum_average = _global_av(_index_penergy);
    sum_ave2 = _global_av2(_index_penergy);
    coutf << setw(12) << blk << "\t"
  ...
  // KINETIC ENERGY ////////////////////////////////////////////////////////////
  if (_measure_kenergy){
    coutf.open(output_path+"OUTPUT/kinetic_energy.dat",ios::app);
    average  = _average(_index_kenergy);
    sum_average = _global_av(_index_kenergy);
    sum_ave2 = _global_av2(_index_kenergy);
    coutf << setw(12) << blk << "\t"
  ...
  // TOTAL ENERGY //////////////////////////////////////////////////////////////
  if (_measure_tenergy){
    coutf.open(output_path+"OUTPUT/total_energy.dat",ios::app);
    average  = _average(_index_tenergy);
    sum_average = _global_av(_index_tenergy);
    sum_ave2 = _global_av2(_index_tenergy);
    coutf << setw(12) << blk << "\t"
  ...
  // TEMPERATURE ///////////////////////////////////////////////////////////////
  if (_measure_temp){
    coutf.open(output_path+"OUTPUT/temperature.dat",ios::app);
    average  = _average(_index_temp);
    sum_average = _global_av(_index_temp);
    sum_ave2 = _global_av2(_index_temp);
    coutf << setw(12) << blk << "\t"
  ...
  // TO BE FIXED IN EXERCISE 4
  if(_measure_pressure){
    coutf.open(output_path+"OUTPUT/pressure.dat",ios::app);
    average  = _average(_index_pressure);
    sum_average = _global_av(_index_pressure);
    sum_ave2 = _global_av2(_index_pressure);
    coutf << setw(16) << blk << "\t"
  ...
  // TO BE FIXED IN EXERCISE 7
  if(_measure_gofr){
    double app=0;double app2=0;
    coutf.open(output_path+"OUTPUT/gofr.dat",ios::app);
    for(int i=0; i<_n_bins; i++){
    ...
    _block_counter++;
    if(_block_counter==_nblocks){
      coutf.open(output_path+"OUTPUT/gofr_final.dat",ios::app);
      for(int i=0; i<_n_bins; i++){
        average  = _average(_index_gofr+i);
        sum_average = _global_av(_index_gofr+i);
        sum_ave2 = _global_av2(_index_gofr+i);
  ...
  // TO BE FIXED IN EXERCISE 6
  if(_measure_cv){
    coutf.open(output_path+"OUTPUT/specific_heat.dat",ios::app);
    average  = _average(_index_cv);
    sum_average = _global_av(_index_cv);
    sum_ave2 = _global_av2(_index_cv);
    coutf << setw(12) << blk << "\t"
  ...
  // TO BE FIXED IN EXERCISE 6
  if(_measure_chi){
    coutf.open(output_path+"OUTPUT/susceptibility.dat",ios::app);
    average  = _average(_index_chi);
    sum_average = _global_av(_index_chi);
    sum_ave2 = _global_av2(_index_chi);
    coutf << setw(12) << blk << "\t"
